...
void hex(int num, char *number, int uppercase)
{
  int dgcount = digit16_count(num);
  int index = dgcount - 1;
  char x;
  if(num == 0 && dgcount == 1){
    number[0] = '0';
...
{
  char str_num[digit16_count(num)+1];
  hex(num, str_num,1);
  print_string(str_num);
}
